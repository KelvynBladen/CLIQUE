...
max(dacon$CLIQUE)
gap = ggplot(dacon, aes(MinTempAve > 65, ICI)) +
  geom_boxplot(coef = 200) + ylab("ACONIF ICI") +
  scale_y_continuous(limits = c(-.35, .35), breaks = -3:3 / 10)

gag = ggplot(dacon, aes(MinTempAve > 65, CLIQUE)) +
  geom_boxplot(coef = 200) + ylab("ACONIF CLIQUE") +
  scale_y_continuous(limits = c(-.35, .35), breaks = -3:3 / 10)

gas = ggplot(dacon, aes(MinTempAve > 65, SHAP)) +
  geom_boxplot(coef = 25) + ylab("ACONIF SHAP") +
  scale_y_continuous(limits = c(-.35, .35), breaks = -3:3 / 10)

gal = ggplot(dacon, aes(MinTempAve > 65, LIME)) +
  geom_boxplot(coef = 25) + ylab("ACONIF LIME") +
  scale_y_continuous(limits = c(-.35, .35), breaks = -3:3 / 10)
...
ggsave("lichen_aconif2k2.jpg", dpi = 1200, width = 5.6, height = 4)
